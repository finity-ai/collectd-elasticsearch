...
NODE_STATS = {
    ...
    "indices.segments.index-writer-max-size": Stat(
        "gauge", "nodes.%s.indices.segments.index_writer_max_memory_in_bytes"
    ),
    "indices.segments.index-writer-size": Stat("gauge", "nodes.%s.indices.segments.index_writer_memory_in_bytes"),
    # DOCS
    ...
    "jvm.mem.pools.young.max_in_bytes": Stat("gauge", "nodes.%s.jvm.mem.pools.young.max_in_bytes"),
    "jvm.mem.pools.young.used_in_bytes": Stat("gauge", "nodes.%s.jvm.mem.pools.young.used_in_bytes"),
    "jvm.mem.pools.old.max_in_bytes": Stat("gauge", "nodes.%s.jvm.mem.pools.old.max_in_bytes"),
    "jvm.mem.pools.old.used_in_bytes": Stat("gauge", "nodes.%s.jvm.mem.pools.old.used_in_bytes"),
    # UPTIME
...
DEPRECATED_NODE_STATS = [
    {"major": 2, "minor": 0, "revision": 0, "keys": ["process.mem.share_in_bytes"]},
    {"major": 5, "minor": 0, "revision": 0, "keys": ["indices.segments.index-writer-max-size"]},
    {"major": 6, "minor": 0, "revision": 0, "keys": ["indices.store.throttle-time"]},
]

# Deprecated thread pools by first version in which they were removed
DEPRECATED_THREAD_POOLS = [
    {"major": 2, "minor": 0, "revision": 0, "keys": ["merge", "optimize"]},
    {"major": 5, "minor": 0, "revision": 0, "keys": ["suggest", "percolate"]},
    {"major": 6, "minor": 3, "revision": 2, "keys": ["bulk"]},
]

NODE_STATS_ES_2 = {
    "indices.cache.filter.evictions": Stat("counter", "nodes.%s.indices.query_cache.evictions"),
    "indices.cache.filter.size": Stat("gauge", "nodes.%s.indices.query_cache.cache_size"),
    "indices.cache.filter.hit-count": Stat("counter", "nodes.%s.indices.query_cache.hit_count"),
    "indices.cache.filter.miss-count": Stat("counter", "nodes.%s.indices.query_cache.miss_count"),
    "indices.cache.filter.cache-count": Stat("counter", "nodes.%s.indices.query_cache.cache_count"),
    "indices.cache.filter.total-count": Stat("counter", "nodes.%s.indices.query_cache.total_count"),
    "indices.search.scroll-time": Stat("counter", "nodes.%s.indices.search.scroll_time_in_millis"),
    "indices.search.scroll.total": Stat("counter", "nodes.%s.indices.search.scroll_total"),
    "indices.search.scroll.current": Stat("gauge", "nodes.%s.indices.search.scroll_current"),
}

# ElasticSearch 6.0.0
NODE_STATS_ES_6 = {
    "indices.translog.uncommitted_operations": Stat("gauge", "nodes.%s.indices.translog.uncommitted_operations"),
    "indices.translog.uncommitted_size_in_bytes": Stat("gauge", "nodes.%s.indices.translog.uncommitted_size_in_bytes"),
}

# ElasticSearch 6.3.0
NODE_STATS_ES_6_3 = {
    "indices.flush.periodic": Stat("gauge", "nodes.%s.indices.flush.periodic"),
    "indices.translog.earliest_last_modified_age": Stat(
        "gauge", "nodes.%s.indices.translog.earliest_last_modified_age"
    ),
}
...
INDEX_STATS_ES_1_3 = {
    # SEGMENTS
    "indices[index={index_name}].primaries.segments.index-writer-memory": Stat(
        "gauge", "primaries.segments.index_writer_memory_in_bytes"
    ),
    "indices[index={index_name}].primaries.segments.version-map-memory": Stat(
        "gauge", "primaries.segments.version_map_memory_in_bytes"
    ),
}
...
INDEX_STATS_ES_6 = {
    # SUGGEST
    "indices[index={index_name}].primaries.flush.periodic": Stat("gauge", "primaries.flush.periodic"),
    "indices[index={index_name}].primaries.translog.earliest_last_modified_age": Stat(
        "gauge", "primaries.translog.earliest_last_modified_age"
    ),
    "indices[index={index_name}].primaries.translog.uncommitted_operations": Stat(
        "gauge", "primaries.translog.uncommitted_operations"
    ),
    "indices[index={index_name}].primaries.translog.uncommitted_size_in_bytes": Stat(
        "gauge", "primaries.translog.uncommitted_size_in_bytes"
    ),
    "indices[index={index_name}].total.flush.periodic": Stat("counter", "total.flush.periodic"),
    "indices[index={index_name}].total.translog.earliest_last_modified_age": Stat(
        "counter", "total.translog.earliest_last_modified_age"
    ),
    "indices[index={index_name}].total.translog.uncommitted_operations": Stat(
        "counter", "total.translog.uncommitted_operations"
    ),
    "indices[index={index_name}].total.translog.uncommitted_size_in_bytes": Stat(
        "counter", "total.translog.uncommitted_size_in_bytes"
    ),
}
...
INDEX_STATS = {
    ...
    # ID_CACHE
    "indices[index={index_name}].primaries.id-cache.memory-size": Stat(
        "gauge", "primaries.id_cache.memory_size_in_bytes"
    ),
    # FLUSH
    ...
    ".total-time": Stat("counter", "primaries.warmer.total_time_in_millis"),
    ...
    "indices[index={index_name}].primaries.fielddata.memory-size": Stat(
        "gauge", "primaries.fielddata.memory_size_in_bytes"
    ),
    "indices[index={index_name}].primaries.fielddata.evictions": Stat("counter", "primaries.fielddata.evictions"),
    # REFRESH
    "indices[index={index_name}].primaries.refresh.total-time": Stat(
        "counter", "primaries.refresh.total_time_in_millis"
    ),
    ...
    "indices[index={index_name}].primaries.merges.current-size": Stat(
        "gauge", "primaries.merges.current_size_in_bytes"
    ),
    # COMPLETION
    ...
    "indices[index={index_name}].primaries.percolate.memory-size": Stat(
        "gauge", "primaries.percolate.memory_size_in_bytes"
    ),
    ...
    "indices[index={index_name}].primaries.store.throttle-time": Stat(
        "counter", "primaries.store.throttle_time_in_millis"
    ),
    # INDEXING
    ...
    "indices[index={index_name}].primaries.indexing.index-time": Stat(
        "counter", "primaries.indexing.index_time_in_millis"
    ),
    ...
    "indices[index={index_name}].total.filter-cache.memory-size": Stat(
        "gauge", "total.filter_cache.memory_size_in_bytes"
    ),
    # FIELDDATA
...
# Thread pool metrics
THREAD_POOL_METRICS = {"gauge": ["threads", "queue", "active", "largest"], "counter": ["completed", "rejected"]}
...
def configure_callback(conf):
    ...
    for node in conf.children:
        if node.key == "Host":
            ...
            log.notice("overriding elasticsearch cluster name to %s" % c.es_cluster_from_config)
    ...
    collectd.register_read(
        read_callback, interval=c.collection_interval, name=get_unique_name(c.es_host, c.es_port, c.es_index), data=c
    )
    log.notice("started elasticsearch plugin with interval = %d seconds" % c.collection_interval)
...
def remove_deprecated_elements(deprecated, elements, version):
    ...
    for dep in deprecated:
        if (
            ...
            or (major == dep["major"] and minor == dep["minor"] and revision >= dep["revision"])
        ):
...
class Cluster(object):
    ...
    def sanitize_intervals(self):
        ...
        if self.index_interval > self.collection_interval:
            ...
            if self.index_interval % self.collection_interval > 0:
                self.index_interval = (
                    self.index_interval + self.collection_interval - (self.index_interval % self.collection_interval)
                )
                log.warning(
                    (
                        "The Elasticsearch Index Interval must be "
        ...
        #   Set self.index_interval = self.collection_interval
            ...
            log.warning(
                (
                    "WARN: The Elasticsearch Index Interval must be "
                    "greater or equal to than and divisible by the "
                    "collection Interval.  The Elasticsearch Index "
                    "Interval has been rounded to: %s"
                )
    ...
    def remove_deprecated_node_stats(self):
        ...
        self.node_stats_cur = remove_deprecated_elements(DEPRECATED_NODE_STATS, self.node_stats_cur, self.es_version)

    def remove_deprecated_threads(self):
        """Remove deprecated thread_pools from the list of stats to collect"""
        self.thread_pools = remove_deprecated_elements(DEPRECATED_THREAD_POOLS, self.thread_pools, self.es_version)
    ...
    def init_stats(self):
        ...
        self.node_stats_cur = dict(list(NODE_STATS.items()))
        self.index_stats_cur = dict(list(INDEX_STATS.items()))
        ...
        if not self.es_index:
            # get all index stats
            self.es_index_url = self.es_url_scheme + "://" + self.es_host + ":" + str(self.es_port) + "/_all/_stats"
        else:
            self.es_index_url = (
                self.es_url_scheme
                + "://"
        ...
        if not self.es_version.startswith("1."):
            thread_pools.extend(
                ["suggest", "percolate", "management", "listener", "fetch_shard_store", "fetch_shard_started"]
            )
        ...
        self.es_cluster_url = self.es_url_scheme + "://" + self.es_host + ":" + str(self.es_port) + "/_cluster/health"

        log.notice(
            "Initialized with version=%s, host=%s, port=%s, url=%s"
    ...
    def fetch_stats(self):
        ...
        if (self.enable_index_stats and self.es_master_eligible and self.skip_count >= self.index_skip) and (
            (self.master_only and self.es_current_master) or (not self.master_only)
        ):
            # Reset skip count
            self.skip_count = 0
            ...
            if indices:
                if self.index_summary_only:
                    ...
                    for index_name in list(indexes_json_stats.keys()):
    ...
    def fetch_url(self, url):
        ...
        try:
            ...
            if self.es_username:
                authheader = base64.encodebytes(
                    ("%s:%s" % (self.es_username, self.es_password)).encode("utf-8")
                ).replace(b"\n", b"")
            ...
            log.error("Error connecting to %s - %r : %s" % (url, e, e))
            return None
        finally:
    ...
    def load_es_info(self):
        json = self.fetch_url(self.es_url_scheme + "://" + self.es_host + ":" + str(self.es_port) + "/_nodes/_local")
        if json is None:
            return

        # Identify the current node
        self.node_id = list(json["nodes"].keys())[0]
        log.notice("current node id: %s" % self.node_id)

        # we should have only one entry with the current node information
        node_info = next(iter(json["nodes"].values()))
        ...
        log.notice(
            "version: %s, cluster: %s, master eligible: %s"
    ...
    def detect_es_master(self):
        ...
        cluster_state = self.fetch_url(
            self.es_url_scheme + "://" + self.es_host + ":" + str(self.es_port) + "/_cluster/state/master_node"
        )
    ...
    def parse_index_stats(self, json, index_name):
        ...
        for name, key in self.index_stats_cur.items():
            ...
            if self.detailed_metrics is True or name.replace("[index={index_name}]", "") in self.defaults:
                result = dig_it_up(json, key.path)
    ...
    def dispatch_stat(self, result, name, key, dimensions=None):
        ...
        log.info(
            ("Parameters to be emitted:\n name: {n}\n key: {k}" "\n dimensions: {d}\n result: {r}").format(
                n=name, k=key, d=dimensions, r=result
            )
        )
        if result is None:
        ...
        if self.es_cluster is None:
            log.warning(
                "Failed to determine Cluster name in read_callback"
        ...
        if self.es_version is None:
            log.warning(
                "Failed to determine ES version in read_callback. "
                + "Will not emit datapoints since plugin_instance "
...
def sanitize_type_instance(index_name):
    ...
    return index_name.replace("/", "_").encode("ascii", "ignore")
...
class CollectdLogHandler(logging.Handler):
    ...
    def emit(self, record):
        ...
        try:
            ...
            collectd.warning(("{p} [ERROR]: Failed to write log statement due " "to: {e}").format(p=self.plugin, e=e))
...
def configure_test(cluster):
    ...
    cluster.configured_thread_pools = set(
        [
            "generic",
            "index",
            "get",
